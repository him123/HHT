 ...
 */
public class IntegrationServiceJSON extends IntentService {
    ...
    public static ArrayList<String> loginUser(Context context, String username, String password, String url) {
        ...
        try {
            ...
            url = url.replaceAll("'","");
            url = url.replaceAll("%20and%20","&");
            url = url.replaceAll("%20eq%20","=");
...
//            if (!Helpers.isNetworkAvailable(context)) {
    ...
    public static String postTrip(Context context, String url, HashMap<String, String> map, JSONArray deepEntity, String purchaseNum) {
...
//            post.addHeader(X_REQUESTED_WITH_KEY, X_REQUESTED_WITH_VAL);
            ...
            if (!Helpers.isNetworkAvailable(context)) {
                ...
                if (response.getStatusLine().getStatusCode() == 201||response.getStatusLine().getStatusCode() == 200) {
                    ...
                    JSONObject jsonObj = new JSONObject(jsonString);
                    //jsonObj = jsonObj.getJSONObject("d");
                    orderId = jsonObj.getString("status");
                    Log.e("Posting", "" + jsonObj);
                    return orderId;
                } else {
                    Log.e("fail", "Fail" + response.getStatusLine().getStatusCode());
    ...
    private static HttpEntity getPayloadBatch(ArrayList<OfflinePost>arrayList) throws IOException {
        ...
        try {
            ...
            int index=1;
            if(arrayList.size()>0){
                OfflinePost offlinePost = arrayList.get(0);
    ...
    public static ArrayList<OfflineResponse> batchRequest(Context context,String collectionName,ArrayList<OfflinePost>arrayList){
        ...
        try {
            DefaultHttpClient client = new DefaultHttpClient();
            client.getCredentialsProvider().setCredentials(getAuthScope(), getCredentials());
            HttpPost post = new HttpPost(collectionName);
//            String authString = "ecs" + ":" + "sap123";
//            byte[] authEncBytes = Base64.encodeBase64(authString.getBytes());
           // post.addHeader("Authorization", "Basic " + new String(authEncBytes));
            post.addHeader(CONTENT_TYPE, APPLICATION_JSON);
            ...
            if(response.startsWith("{")){
                        ...
                        if(json instanceof JSONObject){
                            ...
                            if(checkData){
                                ...
                                if(((JSONObject) json).has("Function"))
                                {
                                    if(((JSONObject) json).getString("Function").equals(ConfigStore.LoadConfirmationFunction)){
                                        if(((JSONObject) json).getString("CustomerId").equals(Settings.getString(App.DRIVER))){
                                            offlineResponse.setFunction(((JSONObject) json).getString("Function"));
                                            offlineResponse.setCustomerID(((JSONObject) json).getString("CustomerId"));
                                            offlineResponse.setOrderID(((JSONObject) json).getString("OrderId"));
                                            Log.e("Response Order","" + ((JSONObject) json).getString("OrderId"));
                                            offlineResponse.setPurchaseNumber(((JSONObject) json).getString("PurchaseNum"));
...
//                                                }
                                ...
                                if(jsonObject.getString("Function").equals(ConfigStore.ReturnsFunction)){
                                        ...
                                        if(jsonObject.getString("DocumentType").equals(ConfigStore.GoodReturnType)){
                                        ...
                                        if(jsonObject.getString("DocumentType").equals(ConfigStore.BadReturnType)){
                                ...
                                if(jsonObject.getString("Function").equals(ConfigStore.AddCustomerFunction)){
                                    offlineResponse.setFunction(jsonObject.getString("Function"));
                                    offlineResponse.setCustomerID(jsonObject.getString("CustomerId"));
                                    offlineResponse.setOrderID(jsonObject.getString("OrderId"));
                                    Log.e("Response Order","" + jsonObject.getString("OrderId"));
                                    offlineResponse.setPurchaseNumber("");
                                    arrayList.add(offlineResponse);
                                }
